...
exports.init = function(appconfig, preHandler) {
  ...
  app.configure(function() {
    ...
    if(config.server.views) {
      ...
      winston.info(pkgname + '  set jade as view engine...path::',config.server.views);
    }
      
    if(config.server.jsonp) {
      app.set('jsonp callback', config.server.jsonp);
      winston.info(pkgname + '  jsonp:',config.server.jsonp);
    }
  ...
  app.configure(function() {
      ...
      if(config.server.path) {
        ...
        winston.info(pkgname + '  static files served (compressed) from:',config.server.path+'/public');
      }
...
function _preHandlers() {
  ...
  if(config.db.enabled) {
    var db = require('./db');
    db.init(config.db);
    winston.info(pkgname + "  mongodb initialized at:",config.db.url);
    ...
    winston.info(pkgname + "  persistent sessions enabled");
  } else {
    session = express.session({
        key    : config.app.key,
        secret : config.app.secret
      });
    winston.info(pkgname + "  in-memory sessions enabled");
  }
  app.use(session);
...
function _postHandlers() {
  ...
  if(config.authentication.enabled) {
    winston.info(pkgname + "  authentication to API's enabled");
    app.all(config.api.path, function(req, res, next) {
      if(config.authentication.ignores && req.params) {
        ...
        if(matches && matches.length>0) {
          winston.debug(pkgname+"ignored auth for param:" + matches);
          return next();
        }
      }
      if(req.loggedIn) {
  ...
  if(config.api.keycheck) {
    winston.info(pkgname + "  keycheck to API's enabled");
  ...
  if(config.server.jsonp) {
    winston.info(pkgname + "  jsonp support on api's enabled");
  ...
  winston.info(pkgname + "  api handler added");
